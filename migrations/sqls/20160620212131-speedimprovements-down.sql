...
update box set grade = null from product
where box.product_code is not null
and product.product_code = box.product_code;
...
drop index if exists loin_idx;
drop index if exists box_idx;
drop index if exists harvester_idx;
drop index if exists lot_idx;
drop index if exists product_idx;
drop index if exists shipping_idx;
drop index if exists supplier_idx;
drop index if exists scan_loin_idx;
drop index if exists scan_box_idx;
drop index if exists scan_lot_idx;
drop index if exists scan_ship_idx;
drop index if exists scan_stn_idx;



drop view if exists loin_totals;
drop view if exists scan_totals;
drop view if exists box_totals;
...
DROP VIEW if exists public.today_total_ship;
DROP VIEW if exists public.today_total_lot;
DROP VIEW if exists public.inventory_detail;
DROP VIEW if exists public.inventory_all;
DROP VIEW if exists public.totals_by_lot;
DROP VIEW if exists public.production_lot;
DROP VIEW if exists public.lot_yield;
DROP VIEW if exists public.lot_summary;
DROP VIEW if exists public.loin_with_info;
DROP VIEW if exists public.box_with_info;
 ...
 SELECT box.box_number,
   ...
   FROM box
     ...
     LEFT JOIN ( SELECT scan_1.box_number,
           ...
           FROM scan scan_1
          GROUP BY scan_1.box_number, scan_1.station_code) scan ON box.box_number::text = 

scan.box_number::text
     LEFT JOIN shipping_unit ON box.shipping_unit_in::text = 
 ...
 SELECT scan.station_code,
   ...
   FROM loin
     ...
     LEFT JOIN ( SELECT scan_1.loin_number,
            ...
            max(scan_1."timestamp") AS "timestamp"
           FROM scan scan_1
          GROUP BY scan_1.loin_number, scan_1.station_code) scan ON scan.loin_number::text = 

loin.loin_number::text;
...
ALTER TABLE public.loin_with_info
 ...
 SELECT scan.lot_number,
   ...
   FROM scan
  WHERE scan.lot_number IS NOT NULL AND scan.lot_number::text <> ''::text AND scan.weight_1 IS NOT 
...
UNION
 SELECT loin_with_info.lot_number,
    ...
    NULL::bigint AS boxes
   FROM loin_with_info
  WHERE loin_with_info.lot_number IS NOT NULL AND loin_with_info.lot_number::text <> ''::text AND 

NOT (loin_with_info.station_code::text IN ( SELECT scan.station_code
           FROM scan
...
ALTER TABLE public.lot_summary
 ...
 SELECT process.process_weight,
    ...
    process.process_weight / process.process_pieces::double precision / (receive.receive_weight / 

receive.receive_pieces::double precision) AS yield_by_pieces,
   ...
   FROM ( SELECT sum(lot_summary.weight_1) AS process_weight,
           ...
           FROM lot_summary,
            lot
          WHERE lot_summary.lot_number::text = lot.lot_number::text AND 

lot_summary.station_code::text = lot.process_station::text
...
lot_summary.station_code::text = lot.receive_station::text
...
ALTER TABLE public.lot_yield
 ...
 SELECT pro_lot.lot_number AS production_lot,
    ...
    lot_yield.total_yield,
        CASE
            WHEN thisfish.tf_code IS NOT NULL AND thisfish.har_response_status IS NULL AND 

thisfish.pro_response_status IS NULL THEN 'submit'::text
            WHEN thisfish.tf_code IS NOT NULL AND thisfish.har_response_status = 201 AND 

thisfish.pro_response_status = 201 THEN 'success'::text
            WHEN thisfish.tf_code IS NOT NULL AND thisfish.har_response_status <> 201 OR 

thisfish.pro_response_status <> 201 THEN 'error'::text
     ...
     LEFT JOIN ( SELECT thisfish_1.tf_code,
        ...
        UNION
         SELECT thisfish_1.label AS tf_code,
          ...
          GROUP BY thisfish_1.lot_number, thisfish_1.label) thisfish ON lot.lot_number::text = 

thisfish.lot_number::text
...
ALTER TABLE public.production_lot
 ...
 SELECT scan.lot_number,
    ...
    NULL::bigint AS boxes
   FROM scan
  WHERE scan.lot_number IS NOT NULL AND scan.lot_number::text <> ''::text AND scan.loin_number IS 

NULL AND scan.box_number IS NULL
...
UNION
 SELECT box_with_info.lot_number,
   ...
   FROM box_with_info
  WHERE box_with_info.lot_number IS NOT NULL AND box_with_info.lot_number::text <> ''::text
  GROUP BY box_with_info.lot_number, box_with_info.station_code, box_with_info.grade, 

box_with_info.product_type, box_with_info.size
UNION
 SELECT loin_with_info.lot_number,
    loin_with_info.station_code,
    sum(loin_with_info.weight_1) AS weight_1,
...
ALTER TABLE public.today_total_lot
 ...
 SELECT box_with_info.shipping_unit_in,
   ...
   FROM box_with_info
  WHERE box_with_info."timestamp"::date = now()::date
  GROUP BY box_with_info.shipping_unit_in, box_with_info.harvester_code, 

box_with_info.station_code, box_with_info.received_from, box_with_info.fleet
  ORDER BY box_with_info.station_code;
