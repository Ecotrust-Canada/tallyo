...
update box set grade = upper(product.product_type) from product
...
create unique index loin_idx on loin (loin_number);
create unique index box_idx on box (box_number);
create unique index harvester_idx on harvester (harvester_code);
create unique index lot_idx on lot (lot_number);
create unique index product_idx on product (product_code);
create unique index shipping_idx on shipping_unit (shipping_unit_number);
create unique index supplier_idx on supplier (supplier_code);


create  index scan_loin_idx on scan (loin_number);
create  index scan_box_idx on scan (box_number);

create  index scan_lot_idx on scan (lot_number);

create  index scan_ship_idx on scan (shipping_unit_number);

create  index scan_stn_idx on scan (station_code);



CREATE OR REPLACE VIEW loin_totals AS 
 SELECT scan.station_code,
    loin.lot_number,
    count(loin.loin_number) AS pieces,
    sum(loin.weight_1) as weight_1,
    loin.grade
   FROM loin
     LEFT JOIN scan ON scan.loin_number::text = loin.loin_number::text
  WHERE scan.pieces > 0
  group by scan.station_code, loin.lot_number, loin.grade;

ALTER TABLE loin_totals
  OWNER TO tuna_processor;


 CREATE OR REPLACE VIEW scan_totals AS 
 SELECT scan.lot_number,
    scan.station_code,
    sum(scan.weight_1) AS weight_1,
    scan.grade,
  ...
  alter table scan_totals owner to tuna_processor;


  CREATE OR REPLACE VIEW box_totals AS 
 SELECT
   ...
   FROM box
     LEFT JOIN scan ON box.box_number::text = scan.box_number::text
  WHERE scan.pieces > 0
  group by scan.station_code, box.station_code, box.lot_number, box.size, box.grade;

ALTER TABLE box_totals
  OWNER TO tuna_processor;
