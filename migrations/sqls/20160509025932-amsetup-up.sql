...
(
  serial_id integer NOT NULL DEFAULT nextval('shipping_id_seq'::regclass),
  sap_code character varying,
  name character varying,
  supplier_code character varying,
  processor_code character varying,
  CONSTRAINT supplier_pk PRIMARY KEY (serial_id),
  CONSTRAINT usup UNIQUE (supplier_code)
);
ALTER TABLE public.supplier
  OWNER TO tuna_processor;


alter table lot add column supplier_code character varying;


alter table lot add constraint suplot foreign key (supplier_code) references supplier (supplier_code);
...
CREATE OR REPLACE VIEW public.harvester_lot AS 
 SELECT lot.lot_number,
   ...
   FROM lot
     ...
     left join supplier on lot.supplier_code = supplier.supplier_code
...
ALTER TABLE public.harvester_lot
  ...
  alter table box add column supplier_code character varying;

  alter table box add constraint supbox foreign key (supplier_code) references supplier (supplier_code);
