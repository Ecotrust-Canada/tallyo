...
CREATE OR REPLACE VIEW public.box_detail AS 
 SELECT scan.station_code,
    ...
    box.case_number,
        CASE
            WHEN h1.ft_fa_code IS NOT NULL AND h1.ft_fa_code != '' AND h1.ft_fa_code != 'NON FT' THEN h1.ft_fa_code
            ELSE 'NFT'
        END AS fair_trade,
    box."timestamp",
...
ALTER TABLE public.inventory_detail
 ...
 SELECT box.box_number,
    ...
    shipping_unit.received_from,
        CASE
            WHEN harvester.ft_fa_code IS NOT NULL AND harvester.ft_fa_code != '' AND harvester.ft_fa_code != 'NON FT' THEN harvester.ft_fa_code
            ELSE 'NFT'
        END AS ft_fa_code,
    harvester.fleet,
